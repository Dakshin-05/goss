...
const ChannelChat = ()  => {
  ...
  const getMessages = async () => {
    ...
    requestHandler(
      async () => await getChannelChatMessages(user.id, serverId, channelId || ""),
      setLoadingMessages,
      (res) => {
        const { data } = res;
        console.log("from the get messages",data)
  ...
  const handleSendMessage = async () => {
    ...
    await requestHandler(
      async () => await sendChannelMessage(
          user.id,
          serverId,
          channelId || "",
          currMessage
  ...
  useEffect(()=>{
    socket?.emit(JOIN_CHANNEL_CHAT_EVENT, channelId);
    setIsConnected(true);
    getMessages();
  },[channelId]);
