...
import { removeBlockedUser } from '../../api';
import { requestHandler } from '../../utils';
import { useAuth } from '../../context/AuthContext';

const BlockedUsers = ({id, name, username, avatar, blocked_at, setIsRequesting }) => {
  const {user} = useAuth()
  const handleRemoveBlockedUser = async() =>{
    await requestHandler(
      async () => await removeBlockedUser(user.id,{blockedId: id}),
      setIsRequesting,
      (res) => {
        console.log(res)
      },
  ...
  return (
    ...
    <div class="font-medium dark:text-white">
     ...
     <div class="text-sm text-gray-500 dark:text-gray-400">at {new Date(blocked_at).toDateString().slice(3)}</div>
   </div>
   </div>
   </div>
       <span class="flex-1 ms-3 whitespace-nowrap"></span>
            <div class="float-right flex flex-row justify-between space-x-50 mr-5">
              ...
              <div class='bg-red-700 w-10 rounded-full flex justify-center items-center h-full hover:cursor-pointer' onClick={handleRemoveBlockedUser}>
